...
! Mike Devereux: this generates only unfeasible solutions in our case    
...
! MD: modified to generate feasible initial solutions...
    !generate initial simplex
    x(:,1) = sol(:)
    do i = 2,Ndim+1
      x(:,i) = sol(:)
      do j = 1,Ndim
        do k = 1,10
          ...
          tmp = x(j,i)
          x(j,i) = 1._rp + 0.1_rp*(2._rp*ran-1._rp)
          x(j,i) = sol(j)*x(j,i)
          if(feasible(x(:,i))) exit
          x(j,i) = tmp
        end do
      end do
    end do
...
!            print*,'performing reflection'
        ...
        if(fr < f(best)) then
            xe = x0 + 2*(xr-x0)
            print*,'performing expansion, best = ',fr
...
!        print*,'shrinking'
    ...
    if(verbose) then 
      write(*, '(A,I5,A)') "Exiting simplex optimization after ",step, &
          " iterations"
    endif
