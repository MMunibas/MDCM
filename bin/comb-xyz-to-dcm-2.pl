...
if(@ARGV+0 != 4){
  die "usage: comb-xyz-to-dcm.pl <fitted-chgs.xyz> <atom-crds.xyz> <frames.txt> <parfile.dcm>\n";
}
...
while(<XYZ>){
  ...
  if($line>2 && $line<=2+$nq){
    if(@a+0 >= 4){
      ...
      $qcoords[$qatm][2]=$a[3];
      $q[$qatm++]=$a[4];
    }else{ die "could not read line $line of charges file\n"; }
  }
...
for(my $i=0; $i<$nq; $i++){
  ...
  for(my $j=0; $j<$natm; $j++){
    ...
    if($tr<$r){
      $hash[$i]=$j;
      $r=$tr;
      if($r<0.00001){ #probably numerical error
        $qcoords[$i][0]=$coords[$j][0];
        $qcoords[$i][1]=$coords[$j][1];
        $qcoords[$i][2]=$coords[$j][2];
      }
    }
  }
