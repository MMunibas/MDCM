...
for i in range(len(sys.argv)):
  if sys.argv[i] == '-in':
    ...
    punxyz = True
  if sys.argv[i] == '-punfile':
    punchfile = sys.argv[i+1]
  if sys.argv[i] == '-lpunfile':
    lpunfile = sys.argv[i+1]
  elif sys.argv[i] == '-h':
...
##########
# Mike Devereux:
# Linearity assignment is not sufficiently robust (fails for e.g. N3-), so run additional
# geometry-based check here. Defining hybridization as SP is sufficient to ensure atoms are
# marked as linear.

lin = [False]*natoms

for i in range(natoms):
  # first test whether atoms with exactly 2 neighbours are colinear with those neighbours
  # A simple way is to calculate unit vectors along both bonds. If they are 180 degrees apart
  # then the sum of the unit vectors will be zero
  if len(neighbour_atoms[i]) == 2:

    v1=[xyzblock[neighbour_atoms[i][0]][1]-xyzblock[i][1], xyzblock[neighbour_atoms[i][0]][2]-xyzblock[i][2], xyzblock[neighbour_atoms[i][0]][3]-xyzblock[i][3]] # bond vector 1
    v1=numpy.divide(v1,numpy.linalg.norm(v1)) # normalize to unit vector

    v2=[xyzblock[neighbour_atoms[i][1]][1]-xyzblock[i][1], xyzblock[neighbour_atoms[i][1]][2]-xyzblock[i][2], xyzblock[neighbour_atoms[i][1]][3]-xyzblock[i][3]] # bond vector 2
    v2=numpy.divide(v2,numpy.linalg.norm(v2)) # normalize to unit vector

    if(numpy.linalg.norm(numpy.add(v1,v2)) < 0.0001): # catch precision errors
      hbrdz[i] = 'SP'


  # now test whether atoms with exactly 1 neighbour and 1 next-neighbour are colinear with those
  # atoms. The same approach is used as above for atoms with exactly 2 neighbours
  if len(neighbour_atoms[i]) == 1:
    if len(nextneighbours[i]) == 1:

      v1=[xyzblock[i][1]-xyzblock[neighbour_atoms[i][0]][1], xyzblock[i][2]-xyzblock[neighbour_atoms[i][0]][2], xyzblock[i][3]-xyzblock[neighbour_atoms[i][0]][3]] # bond vector 1
      v1=numpy.divide(v1,numpy.linalg.norm(v1)) # normalize to unit vector

      v2=[xyzblock[nextneighbours[i][0]][1]-xyzblock[neighbour_atoms[i][0]][1], xyzblock[nextneighbours[i][0]][2]-xyzblock[neighbour_atoms[i][0]][2], xyzblock[nextneighbours[i][0]][3]-xyzblock[neighbour_atoms[i][0]][3]] # bond vector 2
      v2=numpy.divide(v2,numpy.linalg.norm(v2)) # normalize to unit vector
    
      if(numpy.linalg.norm(numpy.add(v1,v2)) < 0.0001): # catch precision errors
        hbrdz[i] = 'SP'
      
# End of correction by Mike Devereux
##########
...
for i in range(len(atypes)):
  com_atype = atypes[i]
  if hbrdz[i] != 'SP':
    for j in priorised_neighbours[i]:com_atype = com_atype + atypes[j] + str(cc)
  else:
