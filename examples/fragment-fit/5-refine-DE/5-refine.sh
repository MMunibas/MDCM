...
# algorithm, here we use DE with a relaxed convergence criterion with the -conv
# flag (see manual for details).
...
# FITTING PARAMETERS
MINNTRY=1      # tries for largest number of charges, here only 1
MAXNTRY=1      # tries for smallest number of charges, here only 1
...
$BINDIR/pcubefit.x -xyz $INITIALXYZ -mtpfile $MTPFILE -conv 0.01 -esp $PCUBE -dens $DCUBE -nacmax $MAXATMCHG -ntry $NTRY -v > $i-charges.out
